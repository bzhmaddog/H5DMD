...
describe('layer placement (resolveLayerPosition)', () => {
    ...
    const resolve = (position: LayerPosition) =>
        resolveLayerPosition('l', position, 20, 8, 100, 40, [], {})
    ...
    test('offsets still apply to the new spellings', () => {
        expect(resolve({hAlign: 'end', hOffset: -5, vAlign: 'start', vOffset: 3}))
            .toEqual({left: 75, top: 3})
    })
})
