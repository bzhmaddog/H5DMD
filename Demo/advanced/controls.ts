...
const layerDescriptors: LayerDescriptor[] = [
    {
        ...
        children: [
            { id: 'bg', kind: 'canvas' },
            { id: 'animation', kind: 'animation' },
        ],
        note: 'Background image + looping animation in one LayerGroup: the common controls act on the group as a unit, each child’s controls are below.',
    },
    {
        id: 'videos',
        label: 'Videos',
        kind: 'group',
        children: [
            { id: 'video-transparent', kind: 'video' },
            { id: 'video-chromakey', kind: 'video' },
        ],
        note: 'Two VideoLayers (transparent webm + chroma-keyed) sharing one slot in a LayerGroup. Both start hidden - use each child’s Visible toggle to pick one.',
    },
    ...
    { id: 'sprite', label: 'Sprite', kind: 'sprites', spriteId: 'scott' },
    {
        ...
        note: 'The "Scott Pilgrim" + "Matthew Patel" corner captions in one LayerGroup spanning the DMD: the common controls act on the group as a unit, each child’s text options are below.',
    },
    {
        id: 'vs',
        label: 'Text: VS',
        ...
        note: 'Two stacked TextLayers ("in" fill + "out" stroke) in one LayerGroup: the common controls act on the group as a unit, each child’s text options are below.',
    },
...
export function buildAdvancedControlPanel(dmd: Dmd): void {
    ...
    addTab('Global (DMD)', panel => {
        ...
        offDotPicker.style.cssText =
            'cursor:pointer;border:1px solid #555;border-radius:4px;height:26px;padding:1px 2px;background:#222;'
        const { r: or, g: og, b: ob } = dmd.offDotColor
        const toHex2 = (x: number) =>
            Math.round(x * 255)
                .toString(16)
                .padStart(2, '0')
        ...
        fpsCheckbox.addEventListener('change', () => {
            dmd.showFPS = fpsCheckbox.checked
        })
        ...
        const hspOf = (r: number, g: number, b: number) => Math.sqrt(0.299 * r * r + 0.587 * g * g + 0.114 * b * b)
        ...
        levelsSelect.style.cssText =
            'background:#222;color:#fff;border:1px solid #555;border-radius:4px;padding:3px 5px;'
        ;[4, 8, 16].forEach(n => {
            const o = document.createElement('option')
            o.value = String(n)
            o.textContent = `${n} levels`
        ...
        hSlider.addEventListener('input', () => {
            curH = parseFloat(hSlider.value)
            syncHsvSliders()
        })
        sSlider.addEventListener('input', () => {
            curS = parseFloat(sSlider.value) / 100
            syncHsvSliders()
        })
    ...
    const textLayerControls = (panel: HTMLDivElement, text: TextLayer) => {
        ...
        row(
            ...
            input,
            btn('Set text', () => {
                if (input.value) text.setText(input.value)
            }),
        )
        ...
        outlineColorInput.value = (
            text.outlineColor.startsWith('#') ? text.outlineColor : '#' + text.outlineColor
        ).slice(0, 7)
    ...
    const bgDrawEditor = (panel: HTMLDivElement, canvas: CanvasLayer) => {
        ...
        const editor = new EditorView({
            doc:
                'function({fillColor, fillGradient, drawGradientRect, drawRect, drawLine, drawBitmap}) { //🔒\n' +
            ...
            panel => {
                ...
                if (desc.kind === 'animation') {
                    ...
                    if (desc.id === 'big-text-demo') {
                        ...
                        borderColorPicker.style.cssText =
                            'cursor:pointer;border:1px solid #555;border-radius:4px;height:26px;padding:1px 2px;background:#222;'
