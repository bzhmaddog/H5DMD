...
export function setupAdvancedLayers(dmd: Dmd, imagesPath: string): void {
    ...
    videos.addLayer(
        ...
        'video-chromakey',
        {
            ...
            loop: true,
            visible: false,
            renderers: [rendererEntry('chroma-key', ChromaKeyRenderer, { color: [0, 0, 0], threshold: 9 })],
        },
        {
    ...
    dmd.addLayer(
        ...
        async matthew => {
            const bitmap = await fetch(matthewImageUrl)
                .then(r => r.blob())
                .then(createImageBitmap)
    ...
    names.addLayer(TextLayer, 'text1', {
        width: 100,
        height: 17,
        text: 'Scott Pilgrim',
        fontSize: 80,
    ...
    names.addLayer(TextLayer, 'text4', {
        width: 95,
        height: 15,
    ...
    dmd.addLayer(
        ...
        async sprite => {
            const bitmap = await fetch(scottSpriteSheetUrl)
                .then(r => r.blob())
                .then(createImageBitmap)
    ...
    dmd.addLayer(TextLayer, 'big-text-demo', {
        width: 426,
        height: 94,
