...
class AnimationLayer extends BaseLayer {
    ...
    on(event: LayerLifecycleEvent | 'play' | 'pause' | 'stop', handler: LayerLifecycleListener<this>): this {
        if (event === 'play') {
            this._playListeners.push(handler as unknown as (layer: AnimationLayer) => void)
            return this
        }
        if (event === 'pause') {
            this._pauseListeners.push(handler as unknown as (layer: AnimationLayer) => void)
            return this
        }
        if (event === 'stop') {
            this._stopListeners.push(handler as unknown as (layer: AnimationLayer) => void)
            return this
        }
